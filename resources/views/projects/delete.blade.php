@extends('layouts.app', ['title' => 'Delete Project'])

@section('content')
<div class="mb-6">
    <h1 class="page-title text-2xl font-bold text-slate-800">Delete Project</h1>
</div>
...
<div class="bg-white rounded-[20px] shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] border border-slate-200/60 ring-1 ring-slate-900/5 transition-all duration-300 hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.08)] hover:-translate-y-0.5 max-w-2xl overflow-hidden">
    <div class="p-6 bg-rose-50 border-b border-rose-100 flex items-start gap-4">
        ...
        <div>
            ...
            <p class="text-sm text-rose-700 mt-1">This action cannot be undone. This will permanently delete the project <strong>{{ $project->project_name }}</strong> and remove it from all associated allocations.</p>
        </div>
    </div>
    ...
    <div class="p-6">
        <dl class="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-6">
            ...
            <div>
                ...
                <dd class="mt-1 text-sm text-slate-900">{{ $project->status }}</dd>
            </div>
        </dl>

        <form method="post" action="{{ route('projects.destroy', $project) }}" class="mt-8 flex items-center gap-3 pt-4 border-t border-slate-100">
            @csrf
            <button type="submit" class="bg-rose-600 hover:bg-rose-700 text-white font-medium py-2 px-4 rounded-md shadow-sm text-sm">Yes, Delete Project</button>
